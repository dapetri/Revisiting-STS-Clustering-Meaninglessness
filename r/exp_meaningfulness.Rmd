...
for (w in 1:length(ws)) {
  ...
  for (k in 1:length(ks)) {
    m <- calculate_meaningfulness(current_ts,opposing_ts,n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify,seed_)
    m <- round(m, 2)
...
for (w in 1:length(ws)) {
  j <- 1
  for (k in 1:length(ks)) {
    m <- calculate_meaningfulness(spx_short, create_random_walk(spx_short, seed_), n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify,seed_)
    print(paste('Calculated: spx_short',m))
    
    m_ <- calculate_meaningfulness(spx_long, create_random_walk(spx_long, seed_), n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify,seed_)
    m <- m + m_
