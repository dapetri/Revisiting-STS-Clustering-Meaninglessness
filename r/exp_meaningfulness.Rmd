...
# spx_short, spx_long, min_daily_temp, bip_de, dax_alltime, bitcoin_alltime, hydraulic, quality

norm_method <- norm_methods[4]
...
for (w in 1:length(ws)) {
  ...
  for (k in 1:length(ks)) {
    m <- calculate_meaningfulness(current_ts,opposing_ts,n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify)
...
```{r}
# spx_short, spx_long, min_daily_temp, dax_alltime, bitcoin_alltime, hydraulic, quality
i <- 1
j <- 1
for (w in 1:length(ws)) {
  j <- 1
  for (k in 1:length(ks)) {
    m <- calculate_meaningfulness(spx_short, (spx_short), n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify)
    print('Calculated: spx_short')
    m <- m + calculate_meaningfulness(spx_long, (spx_long), n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify)
    print('Calculated: spx_long')
    m <- m + calculate_meaningfulness(min_daily_temp, (min_daily_temp), n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify)
    print('Calculated: min_daily_temp')
    m <- m + calculate_meaningfulness(dax_alltime, (dax_alltime), n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify)
    print('Calculated: dax_alltime')
    m <- m + calculate_meaningfulness(bitcoin_alltime, (bitcoin_alltime), n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify)
    print('Calculated: bitcoin_alltime')
    m <- m + calculate_meaningfulness(hydraulic, (hydraulic), n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify)
    print('Calculated: hydraulic')
    m <- m + calculate_meaningfulness(air_quality, (air_quality), n,ks[k],ws[w],r,dist_metric,norm_method,cluster_algo,reduced_sampling,dim_red,unify)
    print('Calculated: air_quality')
    m <- m/7
    print(m)
